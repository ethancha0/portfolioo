...
export function AccordionCardStrip({
  ...
  return (
    <div
      className={cn("flex h-[220px] w-full gap-1.5 sm:h-[260px]", className)}
      ...
      {items.map((item, i) => {
        ...
        return (
          <button
            key={`${item.alt}-${i}`}
            type="button"
            className="relative h-full min-w-0 overflow-hidden rounded-2xl border-0 bg-[#ddd] p-0 outline-none"
            style={{
